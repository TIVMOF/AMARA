"""The Snowflake connection, and where the local files it uploads live.

Both uploaders need the same connection and the same PUT, so they share them
here rather than each carrying a copy.

Credentials come from `AMARA_SNOWFLAKE_*` in the project's `.env` (see
`.env.example`). There are no fallbacks in code: a missing key raises
immediately rather than connecting as somebody else.
"""
