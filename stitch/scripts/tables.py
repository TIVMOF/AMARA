...
def crawls(staged_crawls: DataFrame, currencies: Reference) -> DataFrame:
    ...
    # A site crawled twice in a day - a failed run, then a retry - gives two
    # rows. Keep the one that actually collected something: dropDuplicates
    # picks arbitrarily, so an empty run could stand in for a real one and
    # report a retailer with 44,958 products as having stored none.
    best = Window.partitionBy("site", "date").orderBy(col("products_stored").desc())
    return (
        rows
        .withColumn("_rank", row_number().over(best))
        .where(col("_rank") == 1)
        .drop("_rank")
    )
...
def products(staged: DataFrame, brands: Reference, categories: Reference,
    ...
    # products - one product observation per retailer and crawl date.
    #
