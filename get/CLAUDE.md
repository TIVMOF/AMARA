...
`get/` collects clothing catalogues from fashion retailers into JSON
...
star schema — belongs to `../cut/`. If a change would make this folder
...
```bash
python get.py crawl                    # every enabled site (~2h, 50 sites)
python get.py crawl kith brownsfashion # named sites
python get.py crawl kith --max-pages 2 # short run, to eyeball output
python get.py probe someretailer.com   # is the JSON open? prints a sites/*.yaml
python get.py sites                    # what is configured
python get.py collections kith         # what a store publishes, largest first
```
...
**Deduplication is the only interpretation done here, and it is structural**:
