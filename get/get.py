...
python get.py crawl brownsfashion kith   named sites only
python get.py crawl kith --max-pages 2   short run, for a look at the data
python get.py crawl kith --scraped-at STAMP   add a site to an earlier crawl
python get.py probe example.com          can this domain be scraped?
python get.py sites                      what is configured
python get.py collections kith           what a store publishes
python get.py validate                   is the crawl output sound?
python get.py upload                     the crawl JSON -> the RAW stage
python get.py validate-upload            is every crawl file in the stage?
...
def run_crawl(args: argparse.Namespace) -> int:
    ...
    if not sites:
        print("no sites matched; run `python get.py sites` to see what is configured")
        return 1
