...
product record shape, the dimensional model — lives in `../cut/`.

## In a container

`amara-get`, built from the `Dockerfile` here. `sites/*.yaml` ships inside
the image — it is versioned config, so adding a retailer means rebuilding.

`AMARA_STORAGE_DIR` points at the one volume the pipeline shares; this stage
writes the `collected/` directory inside it, which `cut` then reads. Unset, it
falls back to `data/` beside the code, so a checkout is unaffected. The four `AMARA_INGESTION_*` settings have working
...
```bash
docker run --rm --init -v amara_storage:/data amara-get crawl
```
...
```bash
python get.py crawl                       # every enabled site
python get.py crawl brownsfashion kith    # named sites
python get.py crawl kith --max-pages 2    # short run, to eyeball
python get.py probe example.com           # can this site be scraped?
python get.py sites                       # what is configured
python get.py collections kith            # what a store publishes

python get.py validate                    # is what was collected sound?
python get.py validate kith               # one retailer
python get.py upload                      # the crawl JSON -> the RAW stage
python get.py validate-upload             # is every file in the stage?
python get.py cleanup                     # empty data/, once it is uploaded
```

`get.py validate` is a separate command, run after a crawl. It recomputes
...
store actually said are kept, so cut can decide.
...
`python get.py collections <site>` lists what a store publishes, largest
...
```bash
python get.py probe someretailer.com
```
