...
DEFAULT_PROCESSED_ROOT = paths.TRIMMED_ROOT


def datasets(root: Path) -> list[tuple[str, list[Path]]]:
    if not root.is_dir():
        raise SystemExit(
            f"no trimmed directory at {paths.relative(root)}\n"
            f"  Nothing to upload. Build the tables first: `python cut.py`"
        )
...
def upload(processed_root: Path) -> None:
    ...
    if not found:
        raise SystemExit(
            f"no parquet tables under {paths.relative(processed_root)}\n"
            f"  The directory is there but holds no table. Run `python cut.py`\n"
            f"  to build them, or check that cleanup has not already run."
        )
