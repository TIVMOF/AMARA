...
USAGE = "python cut.py validate    every table under the trimmed directory"
...
TABLES = {
    ...
    "dates": Spec(("date",), ("date", "day", "week", "month", "quarter", "season",
                              "year"), ("season",)),
    "products": Spec(("product", "retailer", "date"),
                     ("product", "retailer", "date", "name", "brand"),
...
def check_coverage(tables: dict, dismantled: Path) -> Iterator[Finding]:
    # A site staged but absent from crawls was skipped, which no row count shows.
    if "crawls" not in tables:
        return
    if not dismantled.is_dir():
        yield Finding(WARN, "coverage",
                      f"no dismantled directory at {dismantled}; the retailer "
                      f"cross-check was skipped")
        return
    staged = {p.parent.name.upper() for p in dismantled.glob("*/*") if p.is_dir()}
...
def validate(root: Path, dismantled: Path) -> list[Finding]:
    if not root.is_dir():
    ...
    findings += [*check_values(tables), *check_money(tables),
                 *check_coverage(tables, dismantled)]

    # Not a check: what shows whether a reference file needs a new alias.
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(prog="python cut.py validate",
        description=USAGE, formatter_class=argparse.RawDescriptionHelpFormatter)
    # Order matters: validate() is called positionally from the parsed values.
