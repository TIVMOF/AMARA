...
USAGE = """\
python cut.py validate-upload          every parquet against the stage
python cut.py validate-upload products named tables only
"""
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python cut.py validate-upload", description=USAGE,
    ...
    if not args.trimmed.is_dir():
        raise SystemExit(f"error: no processed directory at {args.trimmed}")
    wanted = set(args.tables)
    tables = local_tables(args.trimmed, wanted)
    if not tables:
        raise SystemExit(f"error: no tables in {args.trimmed}"
