...
python gather.py crawl brownsfashion kith    # named sites
python gather.py crawl kith --max-pages 2    # short run, to eyeball
python gather.py probe example.com           # can this site be scraped?
...
store actually said are kept, so stitch can decide.
...
`python gather.py collections <site>` lists what a store publishes, largest
...
```bash
python gather.py probe someretailer.com
```
