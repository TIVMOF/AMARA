...
`gather/` collects clothing catalogues from fashion retailers into JSON
...
star schema — belongs to `../stitch/`. If a change would make this folder
...
```bash
python gather.py crawl                    # every enabled site (~2h, 50 sites)
python gather.py crawl kith brownsfashion # named sites
python gather.py crawl kith --max-pages 2 # short run, to eyeball output
python gather.py probe someretailer.com   # is the JSON open? prints a sites/*.yaml
python gather.py sites                    # what is configured
python gather.py collections kith         # what a store publishes, largest first
```
