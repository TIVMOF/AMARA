...
Nothing is cleaned or renamed here — that is `../stitch/`. This stage
...
a row, which is why `stitch` keys on `(variant, product, retailer, date)`.

One pipeline execution has one raw crawl waiting here. If more than one raw
crawl is present, the command fails and names the explicit path to retry with;
it never combines them. `data/` is gitignored.
