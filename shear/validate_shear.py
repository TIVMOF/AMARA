...
def inspect(at: str, directory: Path) -> tuple[list[Finding], int, int]:
    ...
    findings = [Finding(level, at, message) for level, count, message in (
        ...
        # Not shear's fault - what the store served, carried faithfully.
...
def validate(only: list[str] | None, staging: Path, raw: Path) -> list[Finding]:
    if not staging.is_dir():
        raise SystemExit(f"error: no sheared directory at {staging}")
    wanted = set(only or [])
    ...
    if not sites:
        raise SystemExit(f"error: no sheared sites in {staging}")
    ...
    # A crawl never sheared is not a small table to Spark, it is an absent one.
    # gather names its files <retailer>-<stamp>.json; shear writes <site>/<stamp>/.
    if not wanted and not raw.is_dir():
        findings.append(Finding(WARN, "coverage", f"no gather directory at {raw}"))
    elif not wanted:
        done = {(p.parent.name, p.name) for p in staging.glob("*/*") if p.is_dir()}
        crawled = {tuple(p.stem.rsplit("-", 1)) for p in raw.glob("*.json")}
        findings += [Finding(WARN, "coverage", f"{len(names)} {label}: "
                             + ", ".join(f"{s}/{w}" for s, w in sorted(names)[:6])
                             + (" ..." if len(names) > 6 else ""))
                     for label, names in (("never sheared", crawled - done),
                                          ("sheared with no raw file", done - crawled))
                     if names]
