...
USAGE = """\
python sew.py validate-loaded            the most recent crawl, both layers
python sew.py validate-loaded --date D   an earlier crawl date
"""
...
def check_analytical(cursor, db: str, processed: str, analytical: str,
    ...
    # than a broken load - the same gap the cut vocabularies report.
    if facts:
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python sew.py validate-loaded", description=USAGE,
